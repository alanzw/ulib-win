...
// The IExample3 object ////////////////////////////////////////////////////////////

// In our .H file, we use a macro which defines our IExample3 struct
// as so:
//
// typedef struct {
//    IExample3Vtbl  *lpVtbl;
// } IExample3;
//
// In other words, the .H file defines our IExample3 to have nothing
// but a pointer to its VTable. And of course, every COM object must
// start with a pointer to its VTable.
//
// But we actually want to add some more members to our IExample3.
// We just don't want any app to be able to know about, and directly
// access, those members. So here we'll define a MyRealIExample3 that
// contains those extra members. The app doesn't know that we're
// really allocating and giving it a MyRealIExample3 object. We'll
// lie and tell it we're giving a plain old IExample3. That's ok
// because a MyRealIExample3 starts with the same VTable pointer.
//
// We add a DWORD reference count so that this IExample3
...
// Here are IExample3's functions.
//
// Every COM object's interface must have the 3 functions QueryInterface(),
// AddRef(), and Release().
//
// I also chose to add 2, extra functions to IExample3, which a program
// will call with the names GetString and SetString.

// IExample3's QueryInterface()
static HRESULT STDMETHODCALLTYPE QueryInterface(IExample3 *this, REFIID vTableGuid, void **ppv)
{
    // Check if the GUID matches IExample3 VTable's GUID. We gave the C variable name
    // IID_IExample3 to our VTable GUID. We can use an OLE function called
    // IsEqualIID to do the comparison for us. Also, if the caller passed a
    // IUnknown GUID, then we'll likewise return the IExample3, since it can
    ...
    // as an IDispatch too
    if (!IsEqualIID(vTableGuid, &IID_IUnknown) && !IsEqualIID(vTableGuid, &IID_IExample3) && !IsEqualIID(vTableGuid, &IID_IDispatch))
    {
...
// IExample's AddRef()
static ULONG STDMETHODCALLTYPE AddRef(IExample3 *this)
{
    // Increment IExample3's reference count, and return the updated value.
    ...
    // access them). Rather they are defined only above in our MyRealIExample3
    // struct. So typecast to that in order to access those data members
    return(++((MyRealIExample3 *)this)->count);
}

// IExample's Release()
static ULONG STDMETHODCALLTYPE Release(IExample3 *this)
{
    // Decrement IExample3's reference count. If 0, then we can safely free
    // this IExample3 now
    if (--((MyRealIExample3 *)this)->count == 0)
    {
        if (((MyRealIExample3 *)this)->string) SysFreeString(((MyRealIExample3 *)this)->string);
        GlobalFree(this);
...
static HRESULT loadMyTypeInfo(void)
{
    ...
    if (!(hr = LoadRegTypeLib(&CLSID_TypeLib, 1, 0, 0, &pTypeLib)))
    {
        ...
        // our IExample3's VTable, by passing that VTable's GUID
        if (!(hr = pTypeLib->lpVtbl->GetTypeInfoOfGuid(pTypeLib, &IID_IExample3, &MyTypeInfo)))
        {
...
// IExample3's GetTypeInfoCount()
static ULONG STDMETHODCALLTYPE GetTypeInfoCount(IExample3 *this, UINT *pCount)
{
    *pCount = 1;
    return(S_OK);
}

// IExample3's GetTypeInfo()
static ULONG STDMETHODCALLTYPE GetTypeInfo(IExample3 *this, UINT itinfo, LCID lcid, ITypeInfo **pTypeInfo)
{
    register HRESULT    hr;
...
// IExample3's GetIDsOfNames()
static ULONG STDMETHODCALLTYPE GetIDsOfNames(IExample3 *this, REFIID riid, LPOLESTR *rgszNames, UINT cNames, LCID lcid, DISPID *rgdispid)
{
    if (!MyTypeInfo)
    {
        register HRESULT    hr;
...
// IExample3's Invoke()
static ULONG STDMETHODCALLTYPE Invoke(IExample3 *this, DISPID dispid, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *params, VARIANT *result, EXCEPINFO *pexcepinfo, UINT *puArgErr)
{
...
// IExample3's SetString(). This copies the passed string to IExample3's
// string[]
static HRESULT STDMETHODCALLTYPE SetString(IExample3 *this, BSTR str)
{
   ...
   if (!(((MyRealIExample3 *)this)->string = SysAllocStringLen(str, SysStringLen(str))))
        return(E_OUTOFMEMORY);

    return(NOERROR);
}

// IExample3's GetString(). This retrieves IExample3's string[],
// and stores its contents in a buffer passed by the caller
static HRESULT STDMETHODCALLTYPE GetString(IExample3 *this, BSTR *buffer)
{
    // Make sure that caller passed a handle
    if (!buffer) return(E_POINTER);

    // Create a copy of our string. The caller is responsible for freeing it
    if (!(*buffer = SysAllocString(((MyRealIExample3 *)this)->string)))
        return(E_OUTOFMEMORY);

    return(NOERROR);
}

// IExample3's GetPorts(). This creates and returns an IDispatch to allow
...
// Here's IExample3's VTable. It never changes so we can declare it
// static
...
// wants to create and retrieve a pointer to our IExample3
static HRESULT STDMETHODCALLTYPE classCreateInstance(IClassFactory *this, IUnknown *punkOuter, REFIID vTableGuid, void **objHandle)
{
    ...
    *objHandle = 0;
    ...
    {
        // Allocate our IExample3 object (actually a MyRealIExample3)
        if (!(thisobj = (IExample3 *)GlobalAlloc(GMEM_FIXED, sizeof(MyRealIExample3))))
            hr = E_OUTOFMEMORY;
        else
        {
            // Store IExample3's VTable in the object
            thisobj->lpVtbl = (IExample3Vtbl *)&IExample3_Vtbl;
            ...
            ((MyRealIExample3 *)thisobj)->count = 1;

            // Initialize any other members we added to the IExample3. We added
            // a string member
            ((MyRealIExample3 *)thisobj)->string = 0;

            // Fill in the caller's handle with a pointer to the IExample we just
            // allocated above. We'll let IExample3's QueryInterface do that, because
            // it also checks the GUID the caller passed, and also increments the
            // reference count (to 2) if all goes well
            hr = IExample3_Vtbl.QueryInterface(thisobj, vTableGuid, objHandle);
            ...
            IExample3_Vtbl.Release(thisobj);
...
HRESULT PASCAL DllGetClassObject(REFCLSID objGuid, REFIID factoryGuid, void **factoryHandle)
{
    ...
    if (IsEqualCLSID(objGuid, &CLSID_IExample3))
    {
