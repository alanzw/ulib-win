...
public:
    ...
    {
        this->setTitle(_T("UAirfoil Test 0.0.1"));
    ...
    BOOL onCreate()
    {
        ...
        _gbBook->create();
        _gbBook->setWindowText(_T("Airfoils"));
        ...
        _gbData = new AUI::UTransGroupBox(this, -1);
        _gbData->setPos(600, 20, 200, 400);
        _gbData->create();
        _gbData->setWindowText(_T("Data"));

        m_pListCtrlData = new UListView(this, -2);
        m_pListCtrlData->setPos(610, 40, 180, 370);
        m_pListCtrlData->setStyles(LVS_REPORT | LVS_EDITLABELS);
        m_pListCtrlData->create();

        m_pListCtrlData->addColTextWidth(0, _T("NO."), 40);
        m_pListCtrlData->addColTextWidth(1, _T("x"), 50);
        m_pListCtrlData->addColTextWidth(2, _T("y"), 50);

        _airfoil = new UAirfoil(this, 332);
        _airfoil->setPos(150, 500, 600, 200);
        _airfoil->create();
        _airfoil->setWindowText(_T("Data"));
...
private:
    ...
    AUI::UTransGroupBoxP _gbData;
    huys::ADT::UAutoPtr<UListView> m_pListCtrl;
    huys::ADT::UAutoPtr<UListView> m_pListCtrlData;
    ...
    huys::ADT::UAutoPtr<UAirfoil> _airfoil;
private:
    BOOL onMenuAbout()
    {
        this->showMsg(_T("UAirfoil v0.0.1"), _T("About"));
        return FALSE;
    }
};
