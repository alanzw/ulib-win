...
HRESULT IMemStream::Reset( ULONG uSize, BOOL bKeepMem )
{
	HRESULT hr = 0;
	char bSetSize=TRUE;
...
STDMETHODIMP 
IMemStream::SetSize( ULARGE_INTEGER libNewSize )
{
...
STDMETHODIMP 
IMemStream::Commit( DWORD grfCommitFlags )
{
	return E_NOTIMPL;
}
...
STDMETHODIMP 
IMemStream::Clone( IStream **ppstm )
{
	return E_NOTIMPL;
}
...
//
HRESULT IMemStream::Write( BYTE ch )
{
	// expand committed memory if necessary
...
HRESULT IMemStream::Write( WCHAR ch )
{
	// expand committed memory if necessary
