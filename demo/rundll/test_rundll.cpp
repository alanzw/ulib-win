...
LRESULT CALLBACK EmptyWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM
        lParam)
{
        ...
        )
{
    ...
    if (1 == argc)
    {
        printf("DLL filename and arguments are need!\n");
        return 1;
    }
    ...
    if (hDll) {
        ...
        lptMsgBuffer = (LPTSTR)malloc((_tcslen(szMsg) - 2 + _tcslen(lptDllName)
                    + 1) * sizeof(TCHAR));
